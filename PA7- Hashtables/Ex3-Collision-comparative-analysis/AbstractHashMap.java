...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  private int hashValue(K key) {
    ...
    if (wordTable.get(Key)==null){ //if word not in wordTable
      ...
      hashcode = cyclicShiftValue(key); //generate hashcode
  ...
  //Put collisions into a map
  public void collision(int hashcode){
    // if(collisionTable.get(hashcode) != null){
    //   numCollisions++;
    // }
    Integer count = collisionTable.get(hashcode);
    //System.out.println(hashcode);
    // get the previous count for this hashcode
      if (count == null){
...
;        count = 0;  
    ...
    collisionTable.put(hashcode, 1+count);
  }

  public UnsortedTableMap<Integer,Integer> getCollisionTable (){
