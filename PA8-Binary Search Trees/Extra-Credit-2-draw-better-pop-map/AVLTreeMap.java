...
public class AVLTreeMap<K,V> extends TreeMap<K,V> {
 ...
 // @Override
  public Iterable<Position<Entry<K,V>>> entryPositionSet() {
    ArrayList<Position<Entry<K,V>>> buffer = new ArrayList<>(size());
...
public void printPreorder(Position<Entry<K,V>> p, int num){
  
  if (p.getElement() == null){
    return;
  } else{
    for(int i=0 ; i<num;i++){ // print space equal to depth
          System.out.print("* ");
    }
    System.out.println(p.getElement().getKey()); 
        
    if(left(p) != null){
         printPreorder(left(p), num+1);
    }
    if(right(p)!= null){
       printPreorder(right(p), num+1);
    }
