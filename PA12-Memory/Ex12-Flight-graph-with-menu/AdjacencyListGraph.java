...
public class AdjacencyListGraph<V,E> implements Graph<V,E> {
  ...
  /**  GL 11/16/2022 CHANGE iterable edge to arraylist edge
   * Returns an iterable collection of edges for which vertex v is the origin.
   * Note that for an undirected graph, this is the same result
   * returned by incomingEdges(v).
   * @throws IllegalArgumentException if v is not a valid vertex
   */
  public ArrayList<Edge<E>> outgoingEdges(Vertex<V> v) throws IllegalArgumentException {
    InnerVertex<V> vert = validate(v);
    return vert.getOutgoing();   // edges are the values in the adjacency map
  }
  ...
  /**GL CHANGE iterable edge to arraylist edge
   * Returns an iterable collection of edges for which vertex v is the destination.
   * Note that for an undirected graph, this is the same result
   * returned by outgoingEdges(v).
   * @throws IllegalArgumentException if v is not a valid vertex
   */
  public ArrayList<Edge<E>> incomingEdges(Vertex<V> v) throws IllegalArgumentException {
    InnerVertex<V> vert = validate(v);
    return vert.getIncoming();   // edges are the values in the adjacency map
  }

  /** Returns the edge from u to v, or null if they are not adjacent. */
  //MODIFY GL 11/20/2022 
  public Edge<E> getEdge(Vertex<V> u, Vertex<V> v) throws IllegalArgumentException {
    Edge<E> edgeNotFound = null;  //Variable holding edge
    InnerVertex<V> origin = validate(u);
    InnerVertex<V> dest = validate(v);
    ArrayList<Edge<E>> outgoing = origin.getOutgoing();
    ArrayList<Edge<E>> incoming = dest.getIncoming();
    //GL 11/21/2022
    for(Edge<E> edge : outgoing){     //iterate through outgoing edges to find edge containing U-V
      Vertex<V> terminalVertex = endVertices(edge)[1]; //get terminal vertex of edge GL
      if(terminalVertex == dest){                      
         return edge;
      } 
    }
    //Gl 11/21/2022
    for(Edge<E> edge : incoming){//iterate through incoming edges to find edge containing U-V
      Vertex<V> adjacentVertex = endVertices(edge)[0]; //get adjacent vertex of edge
      if(adjacentVertex == dest){                      
         return edge;
      } 
    }
    return   edgeNotFound;   // will be null if no edge from u to v
  }
  ...
  public Edge<E> insertEdge(Vertex<V> u, Vertex<V> v, E element) throws IllegalArgumentException {
    if (getEdge(u,v) == null) {
      ...
      InnerVertex<V> origin = validate(u);
      InnerVertex<V> dest = validate(v);
      origin.getOutgoing().add(e); // change from origin.getOutgoing().put(v, e); GL 11/16/2022
      dest.getIncoming().add(e); // change from dest.getIncoming().put(u, e);  GL 11/16/20220
      return e;
    } else
  ...
  public void removeVertex(Vertex<V> v) throws IllegalArgumentException {
    ...
    for (Edge<E> e : vert.getOutgoing())   //Updated: for(Edge<E> e : vert.getOutgoing().values()) GL 11/16/22
      removeEdge(e);
    for (Edge<E> e : vert.getIncoming())  //Updated: for(Edge<E> e : vert.getOutgoing().values()) GL 11/16/22
      removeEdge(e);
  ...
  private class InnerVertex<V> implements Vertex<V> {
    ...
    private ArrayList<Edge<E>> outgoing, incoming;  //Changed from hashmap GL 11/16/2022
     HashMap<String, Vertex<V>> map= new HashMap<>();

    /** Constructs a new InnerVertex instance storing the given element. */
    public InnerVertex(V elem, boolean graphIsDirected) {
      element = elem;
      outgoing = new ArrayList<Edge<E>>();     //Changed from hashmap GL 11/16/2022
      if (graphIsDirected)
        incoming = new ArrayList<Edge<E>>();   //Changed from hashmap GL 11/16/2022
      else
    ...
    /** Stores the position of this vertex within the graph's vertex list. */   /// changed from position<vertex<v> to index
    public void setPosition( Position<Vertex<V>>  p) { pos = p; } 

    /** Returns the position of this vertex within the graph's vertex list. */   //GL change to int
    public Position<Vertex<V>> getPosition() { return pos; }    

    /** Returns reference to the underlying map of outgoing edges. */   //GL changed to return type 
    public  ArrayList<Edge<E>> getOutgoing() { return outgoing; }

    /** Returns reference to the underlying map of incoming edges. */    //GL changed to return type
    public ArrayList<Edge<E>> getIncoming() { return incoming; }

  public Vertex getDecorator(String a){ // get value of attribute
      return map.get(a); //return map1[a];
    }
  
    public void setDecorator( String a, Vertex d){	// set value
		  map.put(a, d); //map1[a] =d 
	  }
  ...
  private class InnerEdge<E> implements Edge<E> {
    ...
    public void setPosition( Position<Edge<E>>  p) { pos = p; }  ////GL change from postion<edge> to index

    /** Returns the position of this edge within the graph's vertex list. */
    public  Position<Edge<E>> getPosition() { return pos; } ////GL change from postion<edge> to index
